---
layout: post
title: "데이터를 정의하고, 보여주고, 처리하는 각 과정을 독립적으로 분할하는 MVC 디자인패턴"
excerpt: "View에서 특정 이벤트를 수행했을 경우, Controller에서 이벤트에 해당하는 메소드를 Model로 보내고, Model에서 이를 수행한 후 결과를 가져온 뒤, View에 결과를 적용한다는 의미를 갖는 MVC 패턴은 가장 일반적이고 대중적인 디자인 패턴 중 하나다. 본 포스팅에서는 MVC 패턴에 대해 알아보고 이를 Java로 구현해본 간략한 예제 소스를 살펴본다."
description: "Description of the MVC design pattern for the purpose of separation according to each function of data definition, output, and processing."
date: 2022-10-30 15:00:00 +0900
categories: 아키텍처
tags: [design pattern]
keywords: [software architecture, design pattern, mvc]
comments: true
---
...
- Model (무엇을) : 데이터를 정의하고, 정의한 데이터에 대한 로직을 처리하는 용도
- View (어디에) : 모델에서 정의한 데이터를 화면에 보여주는 용도
- Controller (어떻게) : 모델과 뷰를 연결하는 용도
...
MVC 패턴은 개발 과정에서 발생할 수 있는 여러 문제점을 미리 해결하고, 향후 더욱 편리한 유지보수를 위해 만들어진 규칙으로 보면 좋다. 사실 프로그램이 커지고 협업하는 개발자가 많아질수록 가장 어려운 부분이 '협업'일 것이다. 내가 작성한 소스코드를 상대방도 잘 읽을 수 있어야 하고, 상대방이 작성한 내용을 한눈에 읽고 관리하기도 용이해야 한다.
