...
|**구분**|**클라우드 컴퓨팅**|**엣지 컴퓨팅**|
|----|---|---|
|처리<br>위치|데이터를 처리하고 연산하는 데이터센터가 사용자(단말기)로부터 멀리 떨어져 있음|데이터를 처리하고 생성하는 장치(서버)가 사용자(단말기)와 가까운 로컬 기지국에 있음|
|처리<br>방식|중앙 데이터 센터와 사용자(단말기)가 직접 통신|사용자(단말기)와 가까이 위치한 로컬 기지국의 엣지 데이터 센터가 주로 소통하고, 추가적인 작업 혹은 결과물의 저장 등을 위해 로컬 기지국과 중앙 데이터 센터가 통신|
|처리<br>범위|단순 데이터 전송, 저장|데이터 전송 및 저장 뿐만 아니라 컴퓨팅 능력이 필요한 처리 과정 수행|
