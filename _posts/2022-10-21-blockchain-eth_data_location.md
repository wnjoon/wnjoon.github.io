---
layout: post
title: "솔리디티의 레퍼런스 타입 사용시 필요한 데이터 위치 'Storage, Memory, Calldata' 비교"
...
description: "An explanation of the differences between storage, memory, and calldata, which are data location values required when declaring reference type variables in Solidity."
...
## 레퍼런스(Reference) 타입 변수와 데이터 선언 위치

레퍼런스 타입은 다이나믹(Dynamic) 타입이라고도 불리는데, 솔리디티에서 사용하는 데이터의 위치(location)을 명시하지 않으면 컴파일 과정에서 에러를 발생시킨다. 레퍼런스 타입은 아래와 같다.

- Array (솔리디티에서는 bytes, string 타입을 special array로 취급한다)
- Struct
- Mapping

레퍼런스 타입 사용시 선언할 수 있는 데이터 위치 값들은 아래와 같다.
...
- calldata는 함수의 리턴값에서는 사용할 수 없다. 오직 인풋 파라미터에서만 사용가능하다.
...
추가로 함수의 리턴값을 다룰 때에도 calldata와 memory의 차이점이 발생한다.
...
// 오류 : Return argument type struct MatchingEngine.Order storage ref is not implicitly convertible to expected type (type of first return variable) struct MatchingEngine.Order calldata.
```

위의 예시처럼 함수의 리턴에 calldata를 사용하면 오류가 발생한다([StackExchange](https://ethereum.stackexchange.com/questions/117770/how-to-use-calldata-return-values-in-solidity-and-when-are-they-useful)). 위에서 설명한 것 처럼 함수 호출 시 파라미터에 포함되는 데이터를 잠시 저장하는 곳이기 때문에, calldata는 input 파라미터에서만 사용이 가능하다. 리턴 값에는 필히 memory를 선언하도록 하자.
