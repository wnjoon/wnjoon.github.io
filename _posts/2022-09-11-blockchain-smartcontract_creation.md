---
layout: post
title: "스마트 컨트랙트 주소가 만들어지는 방법"
excerpt: "스마트 컨트랙트의 주소는 EOA와 같이 40글자로 이루어지는데, 어떻게 스마트 컨트랙트 주소는 전혀 겹치지 않고 생성될 수 있을까? 바로 스마트 컨트랙트의 주소는 '컨트랙트를 생성하는 EOA의 정보'를 기준으로 만들어지는데, 본 포스팅에서는 스마트 컨트랙트의 생성 과정을 좀더 심도있게 살펴본다."
description: "Description of the smart contract address creation process."
...
스마트 컨트랙트는 '스스로 생성될 수 없고, 스스로 실행될 수도 없는' 특성을 갖는다. 결국 <u>'어떠한 EOA 또는 CA가 만들고 실행'</u>시키는 역할을 수행해야만 하는데, 특이하게도 EOA와 동일하게 40 Bytes 길이의 Hex 값으로 구성된다.
...
> 실제로 스마트 컨트랙트가 직접 실행한다고 보기는 어렵고, EOA로부터 전달된 트랜잭션을 call 또는 delegatecall을 이용하여 대행(전달)한다고 보는 것이 맞다.

여기서 궁금한 점이 생길 것인데, "과연 이 경우 논스는 어떻게 계산하는가?" 이다. 결론부터 말하자면, 스마트 컨트랙트에도 논스가 존재한다.

- [EIP-161](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-161.md#specification)을 기점으로 스마트 컨트랙트의 논스는 0이 아닌 1부터 시작한다.
- 스마트 컨트랙트의 논스는 오직 "해당 컨트랙트가 다른 컨트랙트를 생성하는 경우" 에만 증가한다.
- 스마트 컨트랙트가 다른 스마트 컨트랙트의 '기능'을 호출(internal transaction)하는 경우에는 논스가 증가하지 않는다.
