---
layout: post
title: "이더리움에서 발생하는 이벤트 조회 방법"
excerpt: "이더리움에서는 이벤트(Event)를 통해 스마트 컨트랙트로부터 발생한 결과를 외부로 전달한다. 그렇기 때문에 특정 시간대에 대한 이벤트를 조회하거나, 혹은 이벤트를 계속해서 구독하는 어플리케이션은 이더리움의 모니터링을 위해 꼭 필요하다고 볼 수 있다. 이번 포스팅에서는 이벤트에 대해 간략히 알아보고, 이를 조회하는 어플리케이션을 golang으로 직접 만들어본다."
description: "Develop a function to view events occurring in Ethereum smart contracts using go language and then share the content."
...
이더리움에서 말하는 이벤트는 '로그(log)'를 의미한다. 이더리움에서 트랜잭션이 발생하면, 해당 트랜잭션이 성공하거나 실패하거나 상관없이 트랜잭션 영수증(receipt)을 발행한다. 이 영수증에는 트랜잭션이 실행되는 동안 발생한 여러 상황들에 대한 로그들이 포함되어 있는데, 이벤트는 이러한 로그들을 객체화한 내용이라고 보면 된다.
...
파라미터에 indexed가 추가되어 있다면, 해당 파라미터는 솔리디티 내에서 인덱싱 처리하여 조회의 조건으로 사용된다. 데이터베이스에서 인덱스화 하는 것과 같은 논리인데, 전체 검색 결과 중 특정 조건에 맞는 내용들만 빠르게 필터링 할 수 있도록 지원한다.

IERC20.sol에서 정의한 Transfer 이벤트에 맞게 파라미터를 세팅해서 emit과 함께 호출하면, 실제 transfer 함수를 통해 트랜잭션이 발생할 때 해당 이벤트가 트랜잭션 영수증의 log 부분에 들어가게 된다.
...
실제로 web3js를 이용해 transfer 트랜잭션을 전송한후 발생한 transaction receipt을 확인해보면 아래와 같다.
...
## go언어를 이용한 이벤트 조회 기능 구현

### 1. abigen

abigen으로 만들어진 go 파일 내부를 보면, 해당 스마트 컨트랙트에서 조회할 수 있는 이벤트가 자동으로 생성된다.
...
하지만 abigen을 사용할 경우 아래와 같은 한계점이 존재한다.
...
그래서 이번 포스팅에서 구현해볼 이벤트 조회 기능은 abigen을 사용하지 않고 아래와 같이 구현해볼 계획이다.
...
<p align="center" style="color:gray">
  <img src="https://user-images.githubusercontent.com/39115630/209032140-34428b29-d813-48ec-b559-e434d2b49c32.png" alt="이벤트 구독 어플리케이션의 간략한 도식" />
  <br />
   이벤트 구독 어플리케이션의 간략한 도식
</p><br>
...
#### 5.1. 문자열 형태의 ABI를 실제 사용가능한 타입으로 변환
...
<p align="center" style="color:gray">
  <img src="https://user-images.githubusercontent.com/39115630/209061806-fdca1bcb-fa35-4851-97e0-1498c613912d.png" alt="이벤트 구독에 사용될 토픽의 검색 조건 설정 방식" />
  <br />
   이벤트 구독에 사용될 토픽의 검색 조건 설정 방식
</p><br>
...
#### 5.5. 과거 이력 조회(History) 또는 구독(Subscribe)
...
자세한 구현 결과는 아래 참고자료에 있는 필자의 깃헙 리포지토리([tiny-blockchain-app](https://github.com/wnjoon/tiny-blockchain-app/tree/master/app/pkg/blockchain/event))에서 확인 가능하다.
...
- [tiny-blockchain-app](https://github.com/wnjoon/tiny-blockchain-app/tree/master/app/pkg/blockchain/event)
