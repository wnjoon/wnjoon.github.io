---
layout: post
title:  "이더리움에서 발생하는 이벤트 어플리케이션 만들기" 
...
events라는 항목 아래에 Transfer 이벤트 호출에 대한 결과가 들어있음을 볼 수 있다. 다음부터는 해당 항목을 블록 구간, 스마트 컨트랙트, 이벤트 이름 등에 따라 선택적으로 조회하거나 이를 실시간으로 구독할 수 있는 어플리케이션을 직접 만들고 테스트 해보려고 한다.
...
- {} or nil : 검색조건 없음. 모든 이벤트 결과
- {{A}} : 첫번째 파라미터에 대한 A 조건과 일치하는 이벤트
- {{}, {B}} : 첫번째 파라미터는 모든 값 AND 두번째 파라미터는 B 조건에 일치하는 이벤트
- {{A}, {B}} : 첫번째 파라미터는 A 조건 AND 두번째 파라미터는 B 조건에 일치하는 이벤트
- {{A, B}, {C, D}} : 첫번째 파라미터는 A OR B, AND 두번째 파라미터는 C OR D인 이벤트 
