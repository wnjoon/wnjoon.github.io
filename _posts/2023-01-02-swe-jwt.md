...
excerpt: "웹서비스에는 다양한 인증방식이 존재한다. 그 중에서도 JWT(JSON Web Token)는 특별한 의미를 갖지 않는 문자열로 이루어진 토큰을 이용하여 인증 서버를 직접 접근하지 않더라도 토큰 자체가 보유하고 있는 인증 정보를 이용하는 방식이다. 본 포스팅에서는 JWT에 대해 간략하게 알아보도록 한다."
...
위에 설명한 것 처럼, Jwts 버전 0.10.0 부터 signWith(Key) or signWith(Key, SignatureAlgorithm)이 deprecated 되었다. 기존에는 문자열 형태의 비밀키를 그대로 사용할 수 있었다면, 바뀐 버전부터는 Key 타입으로 변경해서 이를 넣어주어야 한다. 번거로움을 방지하고자 User 객체를 생성할 때 입력받은 Secret Key를 자동으로 Key 타입으로 변경해서 넣어주도록 하였다.
